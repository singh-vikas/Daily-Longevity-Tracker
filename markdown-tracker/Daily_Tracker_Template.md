# Daily Tracker - {{DATE}}

> **Science-Based Longevity Routine** - Track your daily progress to optimize health and longevity

---

## 📊 **Daily Metrics Overview**

### **Sleep Quality**
- [ ] **Bedtime**: _____
- [ ] **Wake Time**: _____
- [ ] **Sleep Duration**: _____ hours
- [ ] **Sleep Quality** (1-10): _____
- [ ] **Notes**: _____

### **Energy Levels**
- [ ] **Morning** (1-10): _____
- [ ] **Afternoon** (1-10): _____
- [ ] **Evening** (1-10): _____
- [ ] **Average**: _____

### **Mood & Stress**
- [ ] **Mood** (1-10): _____
- [ ] **Stress Level** (1-10): _____
- [ ] **Coping Strategies Used**: _____

---

## 🌅 **6:00-6:30 AM — Circadian Reset & Activation Block**

> **Science**: First 2 hours after waking are critical for setting circadian rhythm. Morning sunlight exposure within 30 minutes of waking is the most powerful circadian reset signal.

### **6:00-6:10 AM — Immediate Upon Waking (<2 minutes):**
- [ ] **No phone/screens** (preserve dopamine baseline)
  - [ ] **Why**: Screens immediately upon waking spike dopamine, setting a high baseline that makes natural activities less rewarding
- [ ] **Get out of bed immediately** (avoid snoozing)
  - [ ] **Why**: Snoozing fragments sleep and disrupts circadian rhythm
- [ ] **Morning sunlight exposure** (critical for circadian rhythm):
  - [ ] **Duration**: 5-10 minutes outdoors (bare eyes, no sunglasses)
  - [ ] **Cloudy days**: 10-20 minutes (UV still penetrates clouds)
  - [ ] **Actual time**: _____ minutes
  - [ ] **Why**: Triggers cortisol release, sets circadian clock, improves mood and alertness
  - [ ] **Location**: _____ (outdoor preferred, near window if outdoor not possible)

### **6:10-6:15 AM — Hydration & Gut Activation (5 minutes):**
- [ ] **Warm water with fresh lime** (500ml) - activates digestive enzymes
  - [ ] **Temperature**: Warm (not hot) - optimal for enzyme activation
  - [ ] **Amount**: 500ml
  - [ ] **Lime**: Fresh squeezed (vitamin C, aids digestion)
- [ ] **Optional**: 1 tbsp apple cider vinegar (gut health, blood sugar)
  - [ ] **Diluted in water**: Yes/No
  - [ ] **Benefits**: Improves insulin sensitivity, supports gut microbiome
- [ ] **No caffeine yet** (wait 90-120 min after waking for cortisol optimization)
  - [ ] **Reason**: Cortisol naturally peaks 30-45 min after waking. Caffeine too early can blunt this natural peak and cause afternoon crash

### **6:15-6:30 AM — Mindfulness & Stress Reduction (10-15 minutes):**
- [ ] **Meditation or pranayama**:
  - [ ] **Method**: [ ] Box breathing (4-4-4-4) [ ] Anulom-Vilom [ ] Other: _____
  - [ ] **Duration**: _____ minutes (target: 10-15 min)
  - [ ] **Benefits**: Reduces morning cortisol spike, improves focus, sets calm tone for day
  - [ ] **Location**: _____
  - [ ] **Notes**: _____

---

## 🏋️ **6:30-7:30 AM — Exercise & Performance Block** (60 minutes)

> **Science-Based Protocol**: Morning exercise leverages peak testosterone and muscle protein synthesis. **Log everything you did today — combining modalities is encouraged** (e.g. strength + a short Zone 2 finisher). For fat loss + muscle + cardiac risk (HDL 32 🔴, A1C 5.8% ⚠️), aim **across the week**: **Resistance ×3 · Zone 2 40–45 min ×3–4 · HIIT ×1–2** + daily post-meal walks.

### **6:30-6:40 AM — Pre-Workout Prep (10 min):**
- [ ] **Transition time** (5 min) - Change clothes, prepare equipment, mental shift
- [ ] **L-Tyrosine (500mg)** - optional, before intense training: [ ] Yes [ ] No
  - [ ] **Science**: Enhances focus, dopamine precursor, improves performance under stress
  - [ ] **When to use**: Before strength training or high-intensity sessions
- [ ] **Light hydration** (200-300ml water) - only if needed
  - [ ] **Note**: If you already drank lemon water (500ml), skip unless thirsty
  - [ ] **Reason**: Over-hydration can cause discomfort during exercise
- [ ] **Mental preparation** (2-3 min) - Set workout intention
  - [ ] **Intention**: _____
  - [ ] **Focus areas**: _____

### **6:40-7:20 AM — Workout Session (40 min):**

**Log any/all you did today — strength + Zone 2 on the same day is encouraged for your goals. Weekly targets: Resistance ×3 · Zone 2 ×3–4 · HIIT ×1–2.**

#### **Option A: Strength Training (aim ×3/week — muscle + insulin sensitivity):**
- [ ] **Strength Training**: [ ] Yes [ ] No
- [ ] **Focus**: [ ] Upper body [ ] Lower body [ ] Core [ ] Full body
- [ ] **Protocol**: 
  - [ ] **Exercises**: 3-4 exercises
  - [ ] **Sets**: 3-4 sets per exercise
  - [ ] **Reps**: 8-12 reps (70-80% 1RM)
  - [ ] **Rest**: 60-90 seconds between sets
  - [ ] **Science**: Morning testosterone peak, optimal muscle protein synthesis
- [ ] **Exercises performed**: 
  - [ ] Exercise 1: _____ (Weight: _____ kg, Sets: _____, Reps: _____)
  - [ ] Exercise 2: _____ (Weight: _____ kg, Sets: _____, Reps: _____)
  - [ ] Exercise 3: _____ (Weight: _____ kg, Sets: _____, Reps: _____)
  - [ ] Exercise 4: _____ (Weight: _____ kg, Sets: _____, Reps: _____)
- [ ] **Actual duration**: _____ minutes
- [ ] **Progressive overload**: Track weights, aim for 2-5% increase weekly
- [ ] **Frequency**: 3-4x/week (prevents sarcopenia - biggest aging driver)
- [ ] **RPE (Rate of Perceived Exertion)**: _____/10
- [ ] **Notes**: _____

#### **Option B: Zone 2 Cardio (aim ×3–4/week, 40–45 min — raises HDL, lowers TG):**
- [ ] **Zone 2 Training** (60-70% max heart rate): [ ] Yes [ ] No
  - [ ] **Type**: [ ] Jogging [ ] Cycling [ ] Rowing [ ] Swimming [ ] Other: _____
  - [ ] **Duration**: _____ minutes (target: 30-40 min)
  - [ ] **Heart rate**: _____ bpm (target: 60-70% max HR)
  - [ ] **Science**: Optimizes mitochondrial function, fat oxidation, cardiovascular health
  - [ ] **Frequency**: 3-4x/week, 150-180 min/week total
  - [ ] **Perceived effort**: Should be able to hold conversation
- [ ] **HIIT / Zone 5 (90-100% max HR)** - 4-6 intervals: [ ] Yes [ ] No
  - [ ] **Intervals completed**: _____
  - [ ] **Work duration**: _____ seconds per interval
  - [ ] **Rest duration**: _____ seconds between intervals
  - [ ] **Total duration**: _____ minutes
  - [ ] **Frequency**: 1–2x/week — best exercise lever for low HDL (32 mg/dL) + VO2 max
  - [ ] **Notes**: _____

#### **Option C: Light/Recovery (mobility, 20-30 min):**
- [ ] **Flexibility & Mobility**: [ ] Yes [ ] No
  - [ ] **Surya Namaskar** (5-10 rounds) or dynamic stretching
  - [ ] **OR Light walk** (if recovering)
  - [ ] **Purpose**: Improves range of motion, reduces injury risk, enhances recovery
  - [ ] **Duration**: _____ minutes
  - [ ] **Focus areas**: _____
  - [ ] **Notes**: _____

### **7:20-7:30 AM — Post-Workout Cool-Down (10 min):**
- [ ] **Recovery break before shower**: 
  - [ ] **Wait 10-15 minutes** after workout before showering
  - [ ] **Reason**: Allows body temperature to normalize, prevents rapid cooling (can cause dizziness)
  - [ ] **During this time**: 
    - [ ] Light stretching (5 min) - focus on worked muscles
    - [ ] Hydration (water) - replace fluids lost
    - [ ] Deep breathing/calm down (5 min) - activate parasympathetic nervous system
  - [ ] **Actual wait time**: _____ minutes
  - [ ] **Notes**: _____

---

## 🧴 **7:30-8:00 AM — Shower, Skin Care & Green Tea Block** (30 minutes)

> **Separate block** for skin care and green tea - allows proper transition from exercise to breakfast prep. Timing: 90-120 minutes after waking (optimal cortisol interaction).

### **7:30-7:45 AM — Shower (15 min):**
- [ ] **Cold Exposure** (optional, 2-3x/week): [ ] Yes [ ] No
  - [ ] **Type**: [ ] Cold shower (2-3 min, 50-60°F) [ ] Ice bath (5-10 min)
  - [ ] **Timing**: NOT immediately after strength training (can blunt hypertrophy)
  - [ ] **Better timing**: After cardio or on non-strength days
  - [ ] **Science**: Increases dopamine, norepinephrine, reduces inflammation, improves recovery
  - [ ] **Duration**: _____ minutes
  - [ ] **Temperature**: _____°F
- [ ] **Regular shower**: Warm water, cleanse body
- [ ] **Notes**: _____

### **7:45-7:55 AM — Skin Care Routine (10 min):**
- [ ] **Gentle face wash** with mild cleanser (pH 5.5-6.5)
  - [ ] **Product**: _____
  - [ ] **Why**: Exercise increases skin sensitivity, gentle cleanser prevents irritation
- [ ] **Pat dry** (don't rub - exercise increases skin sensitivity)
- [ ] **Vitamin C serum** (L-ascorbic acid 10-20%) - antioxidant, collagen synthesis
  - [ ] **Product**: _____
  - [ ] **Application**: Apply to face and neck, wait 2-3 min before next step
- [ ] **Moisturizer** (gel-based for oily, cream for dry skin)
  - [ ] **Product**: _____
  - [ ] **Type**: [ ] Gel [ ] Cream
- [ ] **Sunscreen SPF 30+** (zinc oxide or titanium dioxide) - **CRITICAL** for longevity
  - [ ] **Product**: _____
  - [ ] **SPF**: _____
  - [ ] **Why**: UV damage is cumulative and accelerates aging. Daily sunscreen is non-negotiable for longevity.

### **7:55-8:00 AM — Green Tea (5 min):**
- [ ] **Green tea**: [ ] Yes [ ] No
  - [ ] **Timing**: 90-120 minutes after waking (optimal for cortisol interaction)
  - [ ] **Benefits**: 
    - [ ] Antioxidants (EGCG) - powerful anti-inflammatory
    - [ ] L-theanine (calm focus) - reduces jitters from caffeine
    - [ ] Gentle caffeine (20-30mg per cup) - less than coffee
  - [ ] **Note**: ✅ **Good choice** - Green tea supports metabolism, cognitive function, and longevity
  - [ ] **Amount**: _____ ml
  - [ ] **Type**: [ ] Matcha [ ] Sencha [ ] Other: _____
  - [ ] **Notes**: _____

---

## 💼 **8:00-8:45 AM — Deep Work Block** (45 minutes)

> **Optimal Timing**: After green tea and warm shower - body is warm, focused, and ready for peak cognitive performance. This leverages the post-shower warmth and green tea's L-theanine for enhanced concentration.

### **8:00-8:45 AM — Deep Work Session:**
- [ ] **Deep work/study session completed**: [ ] Yes [ ] No
- [ ] **Type**: [ ] Work [ ] Study [ ] Both
- [ ] **Duration**: _____ minutes (target: 45 min)
- [ ] **Most challenging tasks first** (peak cognitive performance)
- [ ] **Tasks completed**: 
  - [ ] Task 1: _____
  - [ ] Task 2: _____
  - [ ] Task 3: _____
- [ ] **Study topics/Work tasks**: _____
- [ ] **Phone in another room** (minimize distractions)
- [ ] **Caffeine timing** (if needed):
  - [ ] Coffee/tea **90-120 minutes after waking** (optimal cortisol interaction)
  - [ ] **L-Theanine pairing** (200mg) - reduces jitters, enhances focus
  - [ ] **Limit**: 100-200mg caffeine (1-2 cups)
  - [ ] **Amount consumed**: _____ mg
- [ ] **Distractions encountered**: _____
- [ ] **Productivity rating**: _____/10
- [ ] **Notes**: _____

---

## 🍳 **8:45-9:15 AM — Breakfast & Metabolic Optimization Block** (30 minutes)

> **Timing**: 2:45-3:00 hours after waking. While slightly later than the typical 90-120 minute window, this timing works well after the morning deep work session and still supports metabolic health. Break fast 16-18 hours after last meal (if doing 16:8 intermittent fasting).

### **8:45-9:00 AM — Breakfast Timing:**
- [ ] **Time**: 8:45-9:00 AM
- [ ] **Break fast 16-18 hours after last meal** (if doing 16:8 intermittent fasting)
  - [ ] **Last meal yesterday**: _____ PM
  - [ ] **Hours fasted**: _____ hours

### **8:45-9:00 AM — High-Protein, Nutrient-Dense Breakfast:**

**Choose one:**

#### **Indian Options:**
- [ ] **Moong dal chilla** with spinach + curd/yogurt
- [ ] **Poha** with peas, peanuts, and vegetables
- [ ] **Vegetable upma** with nuts and seeds
- [ ] **Besan cheela** with chutney + side of sprouts
- [ ] **Sprouts salad** (moong, chickpea) with tomatoes, onions, coriander
- [ ] **Dalia** (broken wheat) with vegetables and nuts
- [ ] **Other/Custom**: _____

#### **Western Options:**
- [ ] **Overnight oats** with berries, chia seeds, walnuts, Greek yogurt
- [ ] **Greek yogurt parfait** with granola, blueberries, almonds
- [ ] **Scrambled eggs** (2-3) with spinach, tomatoes, avocado
- [ ] **Smoothie**: Banana, spinach, almond milk, chia seeds, protein powder
- [ ] **Cottage cheese** with fruit and seeds
- [ ] **Other/Custom**: _____

#### **Breakfast Nutrients Checklist:**
- [ ] **Protein isolate scoop** (whey or pea, ~25g P) - **added daily** to hit the 135g/day protein target for muscle gain
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **Protein**: _____ g (target: 30-40g per meal; **~135g/day total** ≈ 1.9 g/kg for muscle gain) - muscle synthesis, satiety
- [ ] **Fiber**: _____ g (target: 10-15g) - gut health, blood sugar control
- [ ] **Healthy fats**: _____ g (target: 15-20g) - omega-3, MCTs
- [ ] **Complex carbs**: _____ g (target: 40-60g) - sustained energy
- [ ] **Micronutrients**: Vitamin C, B vitamins, magnesium, zinc
- [ ] **Calories**: _____ kcal

### **8:45-9:00 AM — Morning Supplements (with breakfast):**

> **Personalized to Apr 2026 labs** — see [Weekly Diet Plan.md]. 🔴 = essential · ⚪ = optional (keep for future tracking, take only if targeted).
> **Note:** Vitamin C is now taken at the **3:30 PM snack with iron** (better iron absorption), not here.

**🔴 Essential (with breakfast — fat-soluble need food):**
- [ ] **Vitamin D3 (50,000 IU — ONCE WEEKLY, e.g. Sunday)** 🩺 - severe deficiency (10 ng/mL); maintenance 2,000–4,000 IU/day after repletion
  - [ ] **Timing**: With the fattiest meal for absorption
  - [ ] **Taken this week**: [ ] Yes [ ] No
- [ ] **Omega-3 (~2 g EPA/DHA)** 🩺 - lowers TG (227), raises HDL, shifts LDL particle size (800mg is too low for your TG)
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **Creatine monohydrate (5g)** - muscle + strength (any consistent daily time); note: mildly raises creatinine lab value (harmless)
  - [ ] **Taken**: [ ] Yes [ ] No

**⚪ Optional (keep for future tracking):**
- [ ] **CoQ10 (100mg)** - mitochondrial function; may ease statin muscle aches
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **Vitamin K2 (MK-7, 600 mcg)** - directs calcium away from arteries; pairs with high-dose D *(can also take at lunch)*
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **Turmeric/Curcumin (1g)** - anti-inflammatory. ⚠️ **Keep away from the iron dose** (curcumin chelates iron)
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **NR (Nicotinamide Riboside 375mg) or NMN (500mg)** - NAD+ boost (weak human evidence)
  - [ ] **Type**: [ ] NR [ ] NMN
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **Lion's Mane Mushroom** - brain health, memory, focus
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **Spermidine (10mg)** - autophagy, cellular renewal (preliminary evidence)
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] ~~**Vitamin E (67mg)**~~ - ❌ **dropped** (supplemental Vit E is neutral-to-harmful for cardiovascular outcomes)

### **9:00-9:15 AM — Post-Breakfast Walk:**
- [ ] **15-minute walk** (enhances glucose disposal, improves insulin sensitivity)
- [ ] **Continue sunlight exposure** if possible
- [ ] **Walk completed**: [ ] Yes [ ] No
- [ ] **Duration**: _____ minutes
- [ ] **Note**: Can be done during the 10:45-11:00 AM break to avoid interrupting deep work flow
- [ ] **Location**: _____
- [ ] **Notes**: _____

---

## 💼 **9:15-12:00 PM — Deep Work & Cognitive Peak Block**

> **Science**: Peak cognitive performance window. Most challenging tasks first. Use 90-minute focus blocks with 15-minute breaks. This aligns with ultradian rhythms - natural 90-minute cycles of alertness.

### **Cognitive Optimization:**
- [ ] **Caffeine timing** (if needed):
  - [ ] Coffee/tea **90-120 minutes after waking** (optimal cortisol interaction)
  - [ ] **L-Theanine pairing** (200mg) - reduces jitters, enhances focus
  - [ ] **Limit**: 100-200mg caffeine (1-2 cups)
  - [ ] **Amount consumed**: _____ mg
- [ ] **Phone in another room** (minimize distractions)
- [ ] **Environment**: [ ] Quiet [ ] Music [ ] White noise
- [ ] **Temperature**: _____°F/°C (optimal: 68-72°F)

### **9:15-10:45 AM — Deep Work Session 1 (90 min):**
- [ ] **Deep work/study session completed**: [ ] Yes [ ] No
- [ ] **Type**: [ ] Work [ ] Study [ ] Both
- [ ] **Duration**: _____ minutes (target: 90 min)
- [ ] **Most challenging tasks first** (peak cognitive performance)
- [ ] **Study methods used** (if studying):
  - [ ] Active recall (testing yourself)
  - [ ] Spaced repetition (Anki, flashcards)
  - [ ] Feynman technique (explain in simple terms)
  - [ ] Practice problems/exercises
  - [ ] Reading/note-taking
  - [ ] Other: _____
- [ ] **Tasks completed**: 
  - [ ] Task 1: _____
  - [ ] Task 2: _____
  - [ ] Task 3: _____
- [ ] **Study topics/Work tasks**: _____
- [ ] **Focus rating**: _____/10
- [ ] **Distractions**: _____
- [ ] **Notes**: _____

### **10:45-11:00 AM — Break + Snack (15 min):**
- [ ] **Break activities**: 
  - [ ] Stand (avoid sitting for extended periods)
  - [ ] Stretch (full body, 2-3 min)
  - [ ] Hydrate (water)
  - [ ] Rest eyes (20-20-20 rule: every 20 min, look at something 20 feet away for 20 seconds)
- [ ] **Post-breakfast walk** (if not done earlier) - enhances glucose disposal
- [ ] **Sunlight exposure** (if possible) - circadian rhythm support
- [ ] **Hydration**: _____ ml (target: 500-750ml during this block)
- [ ] **Mid-Morning Snack (if needed)**: [ ] Yes [ ] No
  - [ ] **Options**:
    - [ ] Greek yogurt with pomegranate seeds
    - [ ] Mixed nuts (almonds, walnuts, macadamia) - 1-2 oz
    - [ ] Fresh fruit (berries, apple, seasonal)
    - [ ] Cottage cheese with cucumber
    - [ ] Apple slices with almond butter
    - [ ] Roasted chickpeas (protein + fiber)
    - [ ] Dates with walnuts (natural energy boost)
  - [ ] **What I had**: _____
  - [ ] **Calories**: _____ kcal
- [ ] **Green tea** (optional, 2nd cup) - antioxidants, L-theanine
- [ ] **Notes**: _____

### **11:00 AM-12:00 PM — Deep Work Session 2 (60 min):**
- [ ] **Focus work/study session completed**: [ ] Yes [ ] No
- [ ] **Type**: [ ] Work [ ] Study [ ] Both
- [ ] **Duration**: _____ minutes (target: 60 min)
- [ ] **Study methods used** (if studying):
  - [ ] Active recall (testing yourself)
  - [ ] Spaced repetition (Anki, flashcards)
  - [ ] Feynman technique (explain in simple terms)
  - [ ] Practice problems/exercises
  - [ ] Reading/note-taking
  - [ ] Other: _____
- [ ] **Tasks completed**: 
  - [ ] Task 1: _____
  - [ ] Task 2: _____
  - [ ] Task 3: _____
- [ ] **Study topics/Work tasks**: _____
- [ ] **Focus rating**: _____/10
- [ ] **Notes**: _____

---

## 🍽️ **12:00-1:00 PM — Lunch Break & Metabolic Health Block**

> **Timing**: Consistent meal timing regulates circadian rhythm. Eat within 4-6 hour window from breakfast. 12:00-12:30 PM (Prep/Bring/Buy), 12:30-12:45 PM (Eat). Within 4-6 hours from breakfast (8:45 AM breakfast = 12:45 PM latest).

### **12:00-12:15 PM — Pre-Meal Preparation:**
- [ ] **Apple Cider Vinegar + Psyllium husk** (with warm water, before meal)
  - [ ] **ACV amount**: 1 tbsp
  - [ ] **Psyllium husk amount**: _____ g
  - [ ] **Water temperature**: Warm
  - [ ] **Why**: ACV improves insulin sensitivity, psyllium husk provides fiber and slows glucose absorption
  - [ ] **Taken**: [ ] Yes [ ] No

### **12:30-12:45 PM — Lunch Meal:**

#### **Main Course (choose one):**

**Indian Options:**
- [ ] **Grilled fish curry** (salmon, mackerel, sardines) - omega-3 rich
- [ ] **Lentil dal** (moong, masoor, toor) with brown rice
- [ ] **Chicken curry** (lean, grilled) with vegetables
- [ ] **Paneer bhurji** with multigrain roti
- [ ] **Soya chunks curry** with vegetables
- [ ] **Vegetable khichdi** (rice + lentils + vegetables)
- [ ] **Rajma** (kidney beans) with brown rice
- [ ] **Palak tofu curry** with roti
- [ ] **Baingan bharta** with roti
- [ ] **Other/Custom**: _____

**Western Options:**
- [ ] **Grilled chicken breast** with steamed vegetables
- [ ] **Chickpea salad bowl** with quinoa
- [ ] **Tuna salad** with whole grain bread
- [ ] **Quinoa and roasted vegetable bowl**
- [ ] **Lentil stew** with whole grain bread
- [ ] **Other/Custom**: _____

#### **Accompaniments (always include):**
- [ ] **Steamed vegetables**: Broccoli, cauliflower, spinach (cruciferous)
  - [ ] **Why**: Rich in fiber, vitamins, and sulforaphane (anti-cancer compound)
- [ ] **Fermented foods**: Yogurt, pickle (achar), kimchi, sauerkraut
  - [ ] **Why**: Probiotics support gut microbiome, improve digestion
- [ ] **Complex carbs**: Brown rice, multigrain roti, quinoa (if needed)
- [ ] **Salad**: Mixed greens with olive oil dressing
  - [ ] **Why**: Additional fiber, micronutrients, healthy fats

#### **Lunch Nutrients:**
- [ ] **Protein**: _____ g (target: 25-35g)
- [ ] **Carbs**: _____ g
- [ ] **Fats**: _____ g
- [ ] **Fiber**: _____ g (target: 10-15g)
- [ ] **Calories**: _____ kcal

### **12:30-12:45 PM — Lunch Supplements (with meal):**

> **⚪ All optional (keep for future tracking).** Magnesium is now taken in the **evening** (better for sleep + keeps it ≥5h from the 3:30 PM iron). Do **not** take Vitamin K2/calcium near the iron dose.
- [ ] **Vitamin K2 (MK-7, 600 mcg)** ⚪ - calcium regulation, bone, cardiovascular *(can also take at breakfast)*
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **Garlic extract (2.4g equivalent)** ⚪ - cardiovascular health, immune function
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **Magnesium (400mg)** ⚪ - *moved to evening (9 PM) — see Evening Supplements. Only log here if taken at lunch instead.*
  - [ ] **Taken at lunch**: [ ] Yes [ ] No
  - [ ] **Form**: [ ] Glycinate [ ] Citrate [ ] Other: _____
- [ ] **B Complex** (½ pill, Mon & Thurs only) ⚪ - energy metabolism, brain function (B12 was low-normal at 314)
  - [ ] **Day**: _____
  - [ ] **Taken**: [ ] Yes [ ] No

### **12:45-1:00 PM — Post-Lunch:**
- [ ] **10-15 minute walk** (glucose disposal, prevents post-meal energy crash)
  - [ ] **Why**: Light movement after meals improves insulin sensitivity and glucose uptake
  - [ ] **Walk completed**: [ ] Yes [ ] No
  - [ ] **Duration**: _____ minutes
  - [ ] **Location**: _____
- [ ] **Social connection** (if possible) - boosts mood, longevity marker
  - [ ] **With whom**: _____
  - [ ] **Duration**: _____ minutes
- [ ] **Notes**: _____

---

## ☕ **1:00-4:00 PM — Sustained Work & Afternoon Optimization Block**

> **Afternoon Protocol**: Lighter cognitive tasks, collaborative work, energy management. Address the natural 2 PM dip with NSDR and strategic breaks. This aligns with natural circadian rhythm - cortisol and alertness naturally dip in early afternoon.

### **1:00-2:00 PM — Light Work/Study Block (60 min):**
- [ ] **Work/study session completed**: [ ] Yes [ ] No
- [ ] **Type**: [ ] Work [ ] Study [ ] Both
- [ ] **Focus**: Lighter cognitive tasks (email, admin, meetings, review notes)
- [ ] **Tasks completed**: 
  - [ ] Task 1: _____
  - [ ] Task 2: _____
  - [ ] Task 3: _____
- [ ] **Study topics/Work tasks**: _____
- [ ] **Productivity rating**: _____/10
- [ ] **Notes**: _____

### **2:00-2:15 PM — Break, Green Tea & Energy Management (15 min):**
- [ ] **Green tea with lemon** - antioxidants, gentle caffeine, L-theanine
  - [ ] **Science**: Helps manage natural afternoon dip (2-3 PM circadian low)
  - [ ] **Amount**: _____ ml
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **Break activities**: 
  - [ ] Stand, stretch, hydrate
  - [ ] Rest eyes (20-20-20 rule)
  - [ ] Social connection (if possible) - boosts mood, longevity marker
- [ ] **Notes**: _____

### **2:15-3:30 PM — Collaborative Work/Study Block (75 min):**
- [ ] **Work/study session completed**: [ ] Yes [ ] No
- [ ] **Type**: [ ] Work [ ] Study [ ] Both
- [ ] **Focus**: Collaborative work, study groups, or focused learning
- [ ] **Study methods used** (if studying):
  - [ ] Study group discussion
  - [ ] Teaching others (peer tutoring)
  - [ ] Group problem-solving
  - [ ] Review sessions
  - [ ] Other: _____
- [ ] **Tasks completed**: 
  - [ ] Task 1: _____
  - [ ] Task 2: _____
  - [ ] Task 3: _____
- [ ] **Study topics/Work tasks**: _____
- [ ] **Collaboration details**: _____
- [ ] **Notes**: _____

### **3:30-4:00 PM — NSDR & Snack Break (30 min):**

#### **3:30-3:50 PM — NSDR (Non-Sleep Deep Rest) - 10-20 minutes:**
- [ ] **NSDR completed**: [ ] Yes [ ] No
- [ ] **Duration**: _____ minutes (target: 10-20 min)
- [ ] **Benefits**: 
  - [ ] Reduces cortisol
  - [ ] Enhances focus
  - [ ] **Improves learning retention** (critical for students)
- [ ] **Method**: 
  - [ ] YouTube NSDR/Yoga Nidra
  - [ ] Guided meditation
  - [ ] Deep breathing
  - [ ] Other: _____
- [ ] **Science**: Huberman protocol - addresses natural 2-3 PM circadian dip, **consolidates morning learning**
- [ ] **Note**: Critical for study/work performance - do before 4 PM. **Especially important after study sessions for memory consolidation.**
- [ ] **Resource used**: _____
- [ ] **Notes**: _____

#### **3:30-4:00 PM — Mid-Afternoon Snack (IRON ANCHOR — no dairy, no tea/coffee):**

> ⚠️ **This is the daily iron window.** Choose a **dairy-free** snack (skip the Greek-yogurt/nutty-pudding options below — calcium blocks iron). Include a Vitamin C source (orange/berries). **No tea/coffee/green tea within 2h.**
- [ ] **Snack taken**: [ ] Yes [ ] No
- [ ] **Options** *(prefer the dairy-free, Vit-C-containing ones)*:
  - [ ] Nutty pudding (Greek yogurt, flaxseeds, chia seeds, nuts, berries) ⚠️ *dairy — not on iron days*
  - [ ] Fruit chaat (seasonal fruits)
  - [ ] Chia seed pudding with nuts and fruit
  - [ ] Roasted makhana (fox nuts) - protein, low GI
  - [ ] Hummus with carrot sticks
  - [ ] Boiled sweet corn with lemon
  - [ ] Rice cake with almond butter
  - [ ] Dark chocolate (15g) - cognitive benefits, flavonoids
- [ ] **What I had**: _____
- [ ] **Calories**: _____ kcal
- [ ] **Water**: _____ ml

#### **3:30 PM — Afternoon Supplements (IRON ANCHOR — with snack):**

> 🔴 **Essential.** Take iron + Vitamin C together, dairy-free, **≥2h from any tea/coffee** and **not with turmeric/calcium/magnesium**. Recheck ferritin in 8–12 weeks.
- [ ] **Iron — ferrous bisglycinate (25–50mg elemental)** 🩺 - ferritin 14 ng/mL (deficient)
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **Vitamin C (500mg)** - taken here with iron to double non-heme iron absorption
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **No tea/coffee/green tea within 2h of iron**: [ ] Confirmed
- [ ] **Ashwagandha (600mg)** ⚪ - optional; cortisol/stress (can nudge thyroid — recheck TSH)
  - [ ] **Taken**: [ ] Yes [ ] No

#### **Optional Energy Boost:**
- [ ] **Cold exposure** (if not done in morning): [ ] Yes [ ] No
  - [ ] 2-3 minute cold shower
  - [ ] **Why**: Boosts dopamine, norepinephrine, focus
  - [ ] **Duration**: _____ minutes
  - [ ] **Temperature**: _____°F
- [ ] **Notes**: _____

---

## 🚶 **4:00-6:00 PM — Transition & Evening Preparation Block**

### **4:00-5:00 PM — Work Wrap-Up:**
- [ ] **Plan next day** (write 3-4 key tasks)
  - [ ] Task 1: _____
  - [ ] Task 2: _____
  - [ ] Task 3: _____
  - [ ] Task 4: _____
- [ ] **Journal reflection**: 
  - [ ] What went well today? _____
  - [ ] What to improve? _____
  - [ ] Key learnings: _____
- [ ] **Notes**: _____

### **5:00-6:00 PM — Physical Activity:**
- [ ] **Light stretching** or yoga (10-15 minutes): [ ] Yes [ ] No
  - [ ] **Duration**: _____ minutes
  - [ ] **Type**: [ ] Stretching [ ] Yoga [ ] Other: _____
  - [ ] **Focus areas**: _____
- [ ] **Walk or commute** (if applicable): [ ] Yes [ ] No
  - [ ] **Duration**: _____ minutes
  - [ ] **Steps**: _____
- [ ] **OR**: Evening workout session (if morning not possible):
  - [ ] **Evening workout session**: [ ] Yes [ ] No
    - [ ] Strength training: 3-4x/week
    - [ ] Zone 2 cardio: 3-4x/week
    - [ ] **Type**: _____
    - [ ] **Duration**: _____ minutes
    - [ ] **Notes**: _____

### **Pre-Dinner Preparation:**
- [ ] **Hydration**: _____ ml (target: 500-750ml water)
- [ ] **Avoid heavy snacking** (allow insulin to normalize)
- [ ] **Notes**: _____

---

## 🌙 **6:00-9:30 PM — Evening Wind-Down & Sleep Preparation Block**

### **6:00-7:00 PM — Personal Time & Preparation (60 min):**
- [ ] **Evening Review/Study Session** (optional, lighter cognitive load):
  - [ ] **Topic/Subject**: _____
  - [ ] **Duration**: _____ minutes
  - [ ] **Type**: [ ] Review notes [ ] Light reading [ ] System Design [ ] Technical Reading [ ] Other: _____
  - [ ] **Study methods**:
    - [ ] Quick review of morning topics
    - [ ] Spaced repetition review (Anki)
    - [ ] Planning next study session
    - [ ] Light reading (not intensive)
  - [ ] **Completed**: [ ] Yes [ ] No
- [ ] **Plan next day** (write 3-4 key tasks, plan breakfast/lunch/dinner)
  - [ ] Task 1: _____
  - [ ] Task 2: _____
  - [ ] Task 3: _____
  - [ ] Task 4: _____
  - [ ] Breakfast plan: _____
  - [ ] Lunch plan: _____
  - [ ] Dinner plan: _____
- [ ] **Hydration**: _____ ml (target: 500-750ml water)
- [ ] **Notes**: _____

---

### **7:00-8:00 PM — Dinner & Family Time Block (60 min):**

#### **7:00-7:15 PM — Pre-Meal Preparation:**
- [ ] **Apple Cider Vinegar + Psyllium husk** (with warm water, before meal)
  - [ ] **ACV amount**: 1 tbsp
  - [ ] **Psyllium husk amount**: _____ g
  - [ ] **Water temperature**: Warm
  - [ ] **Why**: ACV improves insulin sensitivity, psyllium husk provides fiber and slows glucose absorption
  - [ ] **Taken**: [ ] Yes [ ] No

#### **7:15-7:30 PM — Dinner Timing:**
- [ ] **Time**: 7:00-7:30 PM (optimal timing)
- [ ] **3-4 hours before bedtime** (optimal for sleep quality)
- [ ] **Lighter meal** than lunch (supports circadian rhythm)

#### **7:15-7:30 PM — Dinner Options (choose one):**

**Indian Options:**
- [ ] **Grilled fish/chicken** curry (less oil) with vegetables
- [ ] **Stir-fried vegetables** (bhindi, lauki, broccoli) with dal
- [ ] **Vegetable soup** with whole grain bread
- [ ] **Dal with steamed veggies** and roti
- [ ] **Paneer tikka** with salad
- [ ] **Tofu stir-fry** with mixed vegetables
- [ ] **Methi thepla** with curd
- [ ] **Other/Custom**: _____

**Western Options:**
- [ ] **Grilled salmon** with asparagus and quinoa
- [ ] **Vegetable stir-fry** with tofu
- [ ] **Lentil soup** with whole grain bread
- [ ] **Zucchini noodles** with tomato sauce and lean protein
- [ ] **Baked sweet potato** with beans and vegetables
- [ ] **Veggie omelette** with salad
- [ ] **Other/Custom**: _____

#### **Dinner Accompaniments:**
- [ ] **Steamed vegetables**: Broccoli, cauliflower, leafy greens
- [ ] **Fermented foods**: Sauerkraut, kimchi, yogurt
- [ ] **Small portion** of complex carbs (if needed)
- [ ] **Salad**: Mixed greens with olive oil

#### **Dinner Nutrients Focus:**
- [ ] **Protein**: _____ g (target: 25-35g)
- [ ] **Fiber**: _____ g (target: 10-15g)
- [ ] **Healthy fats**: _____ g (target: 10-15g)
- [ ] **Lower carbs**: _____ g (target: 30-40g - supports sleep)
- [ ] **Calories**: _____ kcal

#### **7:30-8:00 PM — Play with Daughter:**
- [ ] **Quality time**: [ ] Yes [ ] No
- [ ] **Activities**: _____
- [ ] **Duration**: _____ minutes
- [ ] **Notes**: _____

---

### **8:00-8:15 PM — Post-Meal Walk (15 min):**
- [ ] **15-minute walk** (glucose disposal, prevents post-meal energy crash)
- [ ] **Walk completed**: [ ] Yes [ ] No
- [ ] **Duration**: _____ minutes
- [ ] **Light stretching** (5-10 minutes) - can be done during walk
- [ ] **Location**: _____
- [ ] **Notes**: _____

---

### **8:15-8:30 PM — Evening Skin Care Routine (15 min):**
- [ ] **Cleanse face** with gentle cleanser (pH 5.5-6.5)
  - [ ] **Product**: _____
- [ ] **Pat dry** (don't rub)
- [ ] **Retinol or niacinamide serum** (alternate nights):
  - [ ] **Retinol**: Anti-aging, collagen production (use 2-3x/week initially)
    - [ ] **Used tonight**: [ ] Yes [ ] No
    - [ ] **Product**: _____
  - [ ] **Niacinamide**: Pore refinement, anti-inflammatory
    - [ ] **Used tonight**: [ ] Yes [ ] No
    - [ ] **Product**: _____
- [ ] **Moisturizer** (night cream)
  - [ ] **Product**: _____
- [ ] **Eye cream** (if needed)
  - [ ] **Product**: _____
  - [ ] **Used**: [ ] Yes [ ] No
- [ ] **Notes**: _____

---

### **8:30-9:00 PM — Wind-Down Activities Block (30 min):**
- [ ] **Digital sunset begins**: No screens 2 hours before bed
  - [ ] **Last screen time**: _____ PM
  - [ ] **Blue light blocking**: Use blue light glasses or screen filters (if screens needed)
  - [ ] **Dim lights**: Use warm, dim lighting (candles, salt lamps)
  - [ ] **Why**: Blue light suppresses melatonin, delaying sleep onset
- [ ] **Herbal tea**: Chamomile, tulsi (holy basil), or passionflower
  - [ ] **Type**: _____
  - [ ] **Amount**: _____ ml
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **Light reading**: Physical book (not digital)
  - [ ] **Book/Reading**: _____
  - [ ] **Duration**: _____ minutes
  - [ ] **Why**: Physical books don't emit blue light, promote relaxation
- [ ] **Journal & gratitude**: Review day, plan tomorrow breakfast/lunch/dinner and tasks, 3 things grateful for
  - [ ] **Day review**: _____
  - [ ] **Tomorrow's plan**: _____
  - [ ] **Gratitude 1**: _____
  - [ ] **Gratitude 2**: _____
  - [ ] **Gratitude 3**: _____
- [ ] **Gentle stretching or yoga** (if not done earlier): [ ] Yes [ ] No
  - [ ] **Duration**: _____ minutes
  - [ ] **Type**: _____
- [ ] **Notes**: _____

---

### **9:00-9:30 PM — Supplements & Sleep Prep (30 min):**

#### **9:00 PM — Evening Supplements:**

**🔴 Essential:**
- [ ] **Magnesium glycinate (400mg)** - sleep quality + insulin sensitivity; evening slot keeps it ≥5h from the 3:30 PM iron
  - [ ] **Form used**: [ ] Glycinate [ ] Citrate [ ] Other: _____
  - [ ] **Taken**: [ ] Yes [ ] No

**⚪ Optional (keep for future tracking):**
- [ ] **Ashwagandha (600mg)** ⚪ - stress/cortisol *(take at snack OR evening, not both; can nudge thyroid — recheck TSH)*
  - [ ] **Taken**: [ ] Yes [ ] No
- [ ] **L-Theanine (200mg)** ⚪ - if feeling anxious; calm without drowsiness
  - [ ] **Taken**: [ ] Yes [ ] No

#### **10:00 PM — Medication (Rx):**
- [ ] **Atorvastatin (10mg)** 🩺 - **EVERY night, DAILY — not sporadic.** Statins only lower cardiac risk taken daily (liver re-ramps cholesterol overnight). Avoid grapefruit juice. Given Lp(a) 119 + LDL-P 2172, ask your doctor if 10mg is enough or should step up ± ezetimibe.
  - [ ] **Taken tonight**: [ ] Yes [ ] No

#### **Sleep Environment Optimization:**
- [ ] **Cool room**: 65-68°F (18-20°C)
  - [ ] **Actual temperature**: _____°F/°C
  - [ ] **Why**: Lower body temperature promotes deeper sleep
- [ ] **Dark room**: Blackout curtains, eye mask
  - [ ] **Darkness level**: [ ] Complete [ ] Partial
  - [ ] **Why**: Darkness signals melatonin production
- [ ] **Quiet**: Earplugs or white noise machine
  - [ ] **Method**: [ ] Earplugs [ ] White noise [ ] Both [ ] None
  - [ ] **Why**: Reduces sleep disruptions
- [ ] **No electronics**: Phone in another room or airplane mode
  - [ ] **Phone location**: _____
  - [ ] **Mode**: [ ] Airplane mode [ ] Off [ ] Another room
  - [ ] **Why**: Eliminates notifications and EMF exposure
- [ ] **Prepare tomorrow's clothes** (reduces morning decision fatigue)
  - [ ] **Prepared**: [ ] Yes [ ] No
  - [ ] **Why**: Reduces morning stress and decision-making

#### **9:00-9:30 PM — Final Wind-Down:**
- [ ] **Deep breathing or meditation** (5-10 minutes): [ ] Yes [ ] No
  - [ ] **Type**: [ ] Box breathing [ ] 4-7-8 breathing [ ] Body scan [ ] Other: _____
  - [ ] **Duration**: _____ minutes
  - [ ] **Why**: Activates parasympathetic nervous system, promotes relaxation
- [ ] **Final supplements** (if needed):
  - [ ] **Melatonin (300 mcg)** - only if sleep issues persist
    - [ ] **Taken**: [ ] Yes [ ] No
    - [ ] **Note**: Try optimizing sleep environment first
    - [ ] **Why**: Natural sleep hormone, but should be last resort
  - [ ] **Magnesium** (if not taken earlier)
    - [ ] **Taken**: [ ] Yes [ ] No

#### **Sleep Timing:**
- [ ] **Target bedtime**: 9:30-10:00 PM (3-4 hours after dinner at 7:00-7:30 PM)
- [ ] **Actual bedtime**: _____
- [ ] **Target sleep**: 7-9 hours (individual variation)
- [ ] **Wake time**: Consistent (even weekends, ±30 min)
  - [ ] **Target wake time**: _____ AM
- [ ] **Sleep consistency**: Same bedtime/wake time (±30 min) - critical for circadian rhythm
- [ ] **Time in bed**: _____ hours
- [ ] **Notes**: _____

---

## 📊 **Daily Summary & Stats**

### **Hydration**
- [ ] **Water Intake**: _____ liters (target: 2.5-3L)
- [ ] **Other Fluids**: _____ (tea, coffee, etc.)
- [ ] **Total Fluid Intake**: _____ liters

### **Exercise Summary**
- [ ] **Total Exercise Time**: _____ minutes
- [ ] **Steps**: _____
- [ ] **Active Calories**: _____
- [ ] **Workout Type**: [ ] Strength [ ] Cardio [ ] Recovery [ ] None
- [ ] **RPE Average**: _____/10
- [ ] **Notes**: _____

### **Nutrition Summary**
- [ ] **Total Calories**: _____
- [ ] **Protein**: _____ g (target: 1.6-2.2g/kg body weight)
- [ ] **Carbs**: _____ g
- [ ] **Fats**: _____ g
- [ ] **Fiber**: _____ g (target: 30-40g)
- [ ] **Meals Completed**: _____/3
- [ ] **Snacks**: _____
- [ ] **Notes**: _____

### **Habits Completed**
- [ ] **Morning Routine**: [ ] Sunlight [ ] Hydration [ ] Meditation [ ] Exercise
- [ ] **Supplements**: [ ] Morning (D3/Omega-3/Creatine) [ ] Iron+Vit C (3:30, dairy-free) [ ] Evening (Magnesium)
- [ ] **Medication**: [ ] Atorvastatin taken tonight (DAILY — the key adherence metric)
- [ ] **Protein target hit** (~135g/day): [ ] Yes [ ] No
- [ ] **Exercise**: [ ] Yes [ ] No
- [ ] **Meditation/Mindfulness**: [ ] Yes [ ] No
- [ ] **Social Connection**: [ ] Yes [ ] No
- [ ] **Nature Exposure**: [ ] Yes [ ] No
- [ ] **Digital Sunset**: [ ] Yes [ ] No
- [ ] **Sleep Environment**: [ ] Optimized [ ] Not Optimized

---

## 🎯 **Goals & Achievements**

### **Top 3 Wins Today**
1. _____
2. _____
3. _____

### **Challenges Faced**
- _____

### **What I Learned**
- _____

### **Tomorrow's Focus**
1. _____
2. _____
3. _____

---

## 📝 **Notes & Observations**
_____
_____
_____

---

## 🔄 **Weekly Review** (Fill on Sunday)

### **Week of**: _____ to _____

#### **Consistency Metrics**
- [ ] **Days following routine**: _____/7
- [ ] **Average sleep quality**: _____/10
- [ ] **Average energy**: _____/10
- [ ] **Average mood**: _____/10
- [ ] **Exercise days**: _____/7
- [ ] **Meditation days**: _____/7

#### **Health Metrics**
- [ ] **Average sleep duration**: _____ hours
- [ ] **Weight change**: _____ kg
- [ ] **Body measurements**: _____
- [ ] **Other metrics**: _____

#### **Productivity Metrics**
- [ ] **Deep work sessions**: _____
- [ ] **Goals achieved**: _____
- [ ] **Tasks completed**: _____

#### **What Worked Well**
- _____
- _____

#### **What Needs Improvement**
- _____
- _____

#### **Next Week's Goals**
1. _____
2. _____
3. _____

---

*Use this template daily to track your progress and optimize your longevity routine*

*Based on science-backed protocols from Huberman Lab, Peter Attia, Mark Hyman, and Bryan Johnson*
